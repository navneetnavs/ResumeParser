...
function App() {
  ...
  const analyzeResume = async () => {
    ...
    try {
      const response = await fetch('https://resumeparser-production-5539.up.railway.app/match');
      const result = await response.json();
      setData(result); // This now holds your structured JSON object
    } catch (error) {
      console.error("Fetch error:", error);
      alert("Connection failed. Check your Railway backend!");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-12 font-sans text-slate-900">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-10">
          <h1 className="text-2xl font-black tracking-tighter text-indigo-600">RECRUIT.AI</h1>
          {!data && (
             <button 
             onClick={analyzeResume}
             disabled={loading}
             className="bg-indigo-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 disabled:opacity-50"
           >
        ...
        {data ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
              <div>
                <h2 className="text-4xl font-black mb-2">{data.name}</h2>
                <div className="flex gap-4 text-slate-500 font-medium">
                  <span>💼 {data.yearOfExperience} Years Exp</span>
                  <span>💰 {data.salary}</span>
                </div>
              </div>
              <div className="bg-indigo-600 text-white p-6 rounded-2xl text-center min-w-[140px]">
                <div className="text-4xl font-black">{data.matchingJobs[0].matchingScore}%</div>
                <div className="text-[10px] uppercase font-bold tracking-widest opacity-80">Match Score</div>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Left Column: Skills Found */}
              <div className="md:col-span-1 bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Candidate Skills</h3>
                <div className="flex flex-wrap gap-2">
                  {data.resumeSkills.map((skill, i) => (
              ...
              {/* Right Column: JD Alignment */}
              <div className="md:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Requirement Analysis</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {data.matchingJobs[0].skillsAnalysis.map((item, i) => (
                    <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border ${item.presentInResume ? 'border-green-100 bg-green-50/50' : 'border-slate-100 bg-slate-50/30'}`}>
                      <span className="font-bold text-sm text-slate-700">{item.skill}</span>
                      <span className={`text-[10px] font-black px-2 py-1 rounded ${item.presentInResume ? 'text-green-600 bg-green-100' : 'text-slate-300 bg-slate-100'}`}>
                        {item.presentInResume ? 'VERIFIED' : 'MISSING'}
                      </span>
                    </div>
                  ))}
            ...
            <button onClick={() => setData(null)} className="text-slate-400 text-sm font-bold hover:text-slate-600 transition-colors">
              ← Back to Dashboard
            </button>
          </div>
        ) : (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            <p className="text-slate-400 font-medium">Upload a resume to begin analysis</p>
          </div>
        )}
      </div>
    </div>
